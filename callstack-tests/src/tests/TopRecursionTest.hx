...
class TopRecursionTest {
	public static function run() {
		try {
			toprecursion.Bottom.entry();
			...
			final r = new Inspector(stack).inspect([
				#if eval
				// On eval the native exception stack carries the sync throw site
				// (Top.throwing and its callers) and the invokeResume mechanism
				// patches the first coro frame to its actual call position.
